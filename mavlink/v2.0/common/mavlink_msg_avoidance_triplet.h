...
#define MAVLINK_MSG_ID_AVOIDANCE_TRIPLET_CRC 45
#define MAVLINK_MSG_ID_269_CRC 45
...
#define MAVLINK_MESSAGE_INFO_AVOIDANCE_TRIPLET { \
    ...
    {  { "time_usec", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_avoidance_triplet_t, time_usec) }, \
         ...
         { "delta_t", NULL, MAVLINK_TYPE_FLOAT, 0, 48, offsetof(mavlink_avoidance_triplet_t, delta_t) }, \
...
static inline void mavlink_msg_avoidance_triplet_send(mavlink_channel_t chan, uint64_t time_usec, uint32_t gen_count, float prev_x, float prev_y, float prev_z, float ctrl_x, float ctrl_y, float ctrl_z, float next_x, float next_y, float next_z, float delta_t, float max_acc, float acc_per_err)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_AVOIDANCE_TRIPLET_LEN];
    _mav_put_uint64_t(buf, 0, time_usec);
...
/**
 * @brief Get field delta_t from avoidance_triplet message
 *
 * @return Desired time it takes to travel from prev to next in s
 */
static inline float mavlink_msg_avoidance_triplet_get_delta_t(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_avoidance_triplet_decode(const mavlink_message_t* msg, mavlink_avoidance_triplet_t* avoidance_triplet)
{
    ...
    avoidance_triplet->delta_t = mavlink_msg_avoidance_triplet_get_delta_t(msg);
